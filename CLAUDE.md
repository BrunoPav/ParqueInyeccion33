...
| CORS declarado como `CorsConfigurationSource` y enchufado a la cadena con `.cors(...)` | la config de `WebMvcConfigurer` no la ve Spring Security, y la cadena de filtros corre antes que MVC: el `OPTIONS` del preflight caía en `anyRequest().authenticated()` y devolvía 403. Los `GET` simples seguían andando porque no disparan preflight, así que el síntoma era "los listados funcionan y las escrituras no se conectan". **El bean debe llamarse `corsConfigurationSource`**: Spring Security lo busca por ese nombre exacto, no por tipo |
